...
test('Episodes show list when rerender with new data', () => {
    const { queryAllByTestId, rerender } = render(
        <Episodes episodes={[]}/>
    )

    expect(queryAllByTestId("episode_data")).toStrictEqual([])

    rerender(
        <Episodes episodes={mockData} />
    )

    expect(queryAllByTestId("episode_data")).toHaveLength(8)
})